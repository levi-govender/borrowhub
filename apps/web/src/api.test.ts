...
test("list calls the BFF admin catalogue with demo admin headers", async () => {
  const api = createInventoryApi("http://bff.test", async (input, init) => {
    assert.equal(String(input), "http://bff.test/api/v1/admin/equipment?category=phone&checkedOut=true&page=1");
  ...
  const page = await api.list({ category: "phone", checkedOut: true, page: 1 });
