...
export async function buildApp(options: AppOptions = {}): Promise<FastifyInstance> {
  ...
  app.post("/api/v1/bookings/:id/cancel", async (request, reply) => {
    ...
    headers["idempotency-key"] = idempotencyKey;
    try {
      return await backend.cancelBooking(id, headers, traceId, request.body);
    } catch (error) {
      return sendBackendError(reply, error, traceId);
    }
  });
  ...
  app.get("/api/v1/admin/equipment", async (request, reply) => {
    ...
    const query = request.query as Record<string, string | undefined>;
    const search = new URLSearchParams();
    for (const key of ["query", "category", "checkedOut", "loanOverdue", "reserved", "page", "pageSize"] as const) {
      const value = query[key];
      if (value) {
        search.set(key, value);
      }
