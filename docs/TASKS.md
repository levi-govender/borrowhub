...
| ENH-04 | Admin calendar | DONE | Evidence 2026-09-22: Java `from`/`to` overlap on admin bookings; `AdminApiTest` BUILD SUCCESSFUL. BFF 13 tests + typecheck. Web 14 tests (calendar week + half-open) + typecheck + `vite build`. Calendar tab is office Monday–Sunday. No new booking rules. |
...
| ENH-10 | Checked-out inventory | DONE | Evidence 2026-09-23: `AdminApiTest` and `EquipmentCatalogueTest` BUILD SUCCESSFUL. `checkedOut=true` lists only assets with a CHECKED_OUT loan; rows include `checkedOutTo` and `loanOverdue`. Web 15 tests + typecheck. BFF 13 tests. |
| ENH-11 | Reservation window | DONE | Evidence 2026-09-23: `pnpm --filter @borrowhub/mobile test` 12 passed (office local parse + policy window) + typecheck. Detail screen start/end fields. Java still revalidates the booking. Expo UI not run. |
| ENH-12 | Overdue assets | DONE | Evidence 2026-09-23: `AdminApiTest` and `EquipmentCatalogueTest` BUILD SUCCESSFUL. `loanOverdue=true` lists assets whose CHECKED_OUT loan ended before now. Web 15 tests + typecheck. BFF 13 tests. |
| ENH-13 | Open loan from inventory | DONE | Evidence 2026-09-23: `AdminApiTest` and `EquipmentCatalogueTest` BUILD SUCCESSFUL (`checkedOutBookingId`). Web typecheck. Inventory holder name opens that booking. |
| ENH-14 | Next reservation | DONE | Evidence 2026-09-23: `AdminApiTest` and `EquipmentCatalogueTest` BUILD SUCCESSFUL. Admin equipment detail `nextReservation` is the earliest RESERVED booking. Web typecheck. Employee detail leaves it null. |
| ENH-15 | Reserved inventory | DONE | Evidence 2026-09-23: `AdminApiTest` and `EquipmentCatalogueTest` BUILD SUCCESSFUL. `reserved=true` lists assets with a RESERVED booking; rows include `nextReservedTo` and `nextReservedBookingId`. Web 15 tests + typecheck. |
...
| ENH-32 | Home open loans | DONE | Evidence 2026-09-23: mobile typecheck + 14 tests. Home loads checked-out and reserved bookings (up to 100 each) instead of the newest 20 of every status. Returned and cancelled stay off home. Expo UI not run. |
| ENH-33 | Home loan pages | DONE | Evidence 2026-09-23: mobile typecheck + 14 tests. Home follows further pages when checked-out or reserved loans exceed 100. Returned and cancelled stay off home. Expo UI not run. |
