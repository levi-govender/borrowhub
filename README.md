...
Admin web (http://localhost:5173) — dashboard (overdue card opens overdue loans), inventory (create/edit including archived), bookings, calendar (office week in Africa/Johannesburg), overdue, audit (`changeSummary` on booking detail). API failures show `[CODE; trace …]` for log lookup.
